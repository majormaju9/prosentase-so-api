...
const ALFASTORE_URL =
  "https://app.alfastore.co.id/prd/api/sis/transaksi/lpb/lov";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!storeId || !lpbType || !storeDate) {
      return NextResponse.json(
        {
          error: true,
          message: "storeId, lpbType, dan storeDate wajib diisi",
        },
        { status: 400 }
      );
    }
    ...
    // Teruskan response asli dari AlfaStore
    ...
    console.error("ALFASTORE LPB LOV ERROR:", error);

    return NextResponse.json(
      {
        error: true,
        message: "Gagal mengambil data LPB LOV",
      },
      { status: 500 }
    );
  }
